...
const tokenCheck = async (req, res, next) => {
    ...
    await jwt.verify(token, process.env.JWT_SECRET_KEY, async (err, decoded) => {
        ...
        const loginUser = await user.findById(decoded.sub).select("_id name lastname email");

        // console.log(loginUser);

        if (!loginUser) throw new APIError("Geçersiz Token", 401);
...
const decodedTemporaryToken = async (temporaryToken) => {
    ...
    await jwt.verify(token, process.env.JWT_TEMPORARY_KEY, async (err, decoded) => {
        if (err) throw new APIError("Geçersiz Token", 401);
        userInfo = await await user.findById(decoded.sub).select("_id name lastname email");
